...
forplot %>%
  ...
  cor(BV,TGV)
# [1] 0.3655159
# The correlation between predUC BV and TGV is not high.


forplot %>%
  group_by(predOf) %>%
  slice_max(order_by = predUsefulness, n = 50) %>% ungroup() %>%
  distinct(sireID,damID) %>% nrow() # 96
```

The correlation between predUC BV and TGV is not high.

Not many crosses are selected for both $UC_{parent}$ (`predOf=="BV"`) and $UC_{variety}$ (`predOf=="TGV"`).

This is different from what I found in the implementation analyses.
...
#  slice_max(order_by = predUsefulness, n = 1000) %>% 
  ...
  labs(x = "Cross Rank UC_variety",
       ...
       title = "Predicted distribution (mean and variability) of a sample of 2000 of 20100 predicted crosses",
...
-   [**Download the CSV for the top 50 crosses and their selection index means, SDs and UC here**](ftp://ftp.cassavabase.org/marnin_datasets/IITA_2021GS/output/top50crosses.csv) or
-   [**Download the complete CSV of ALL genomic mate predictions here**](ftp://ftp.cassavabase.org/marnin_datasets/IITA_2021GS/output/genomicMatePredictions_2021Aug10.csv): for all crosses and both SELIND and component trait predictions.
