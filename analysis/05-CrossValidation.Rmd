...
```{r debug inputs for computeVarPredAccuracy}
require(tidyverse); require(magrittr); 
...
# markEffs<-markEffsAD
# modelType<-"AD"
# # debug outer loop (all families, current rep-fold-predType)
# # famgblups<-out$famgblups[[1]]
# # gblupmat<-famgblups$gblupmat[[1]]
# selInd=TRUE
```
```{r computeVarPredAccuracy - v2}
# computeVarPredAccuracy<-function(crossValOut,markEffs,ped,modelType,
...
#   gblups<-retrieve_testset_gblups(markEffs)
  # gblups %>% head
  # gblups$testset_gblups[[1]] %>% unnest(gblups) %>% head
...
```
```{r}
# crossValOut<-cvPredVarsA; markEffs<-markEffsA; modelType<-"A"; selInd=TRUE; SIwts=SIwts
#crossValOut<-cvPredVarsAD; markEffs<-markEffsAD; modelType<-"AD"; selInd=TRUE; SIwts=SIwts
```
```{r varPredAccuracy}
